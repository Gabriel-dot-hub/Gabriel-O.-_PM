...
typedef struct  //declara la estructura producto
{
    int clave;
    char nombre[15];
    float precio;
...
void Reabasatecimiento(producto *, int);
    ...
    while(CLA)
    {
        ...
        else
        {
            ...
            if(RES)
            {
                ...
                A[I].existencia = 0; //stock queda en 0
            }
        }
...
void Reabasatecimiento(producto A[], int T)
...
que continúen ordenados. */
{
    ...
    while((*T < 30)  && (CLA))
    {
        ...
        while ((I < *T) && (A[I].clave < CLA))
            ...
            I++;
        if(I == *T)  //elemento en la ultima posición
        {
            A[I].clave = CLA;
            printf("\tNombre:");
            fflush(stdin);
