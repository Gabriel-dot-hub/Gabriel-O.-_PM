...
/* Variables globales. ACT se utiliza para acumular las temperaturas,
 ...
 void main(void)
 {
     ...
     for (I = 1; I <= 24; I++)
     {
         ...
         scanf("%f", &TEM);
         Acutem(TEM);
         Maxima(TEM, I); //llamada de la funciones pasa los parametros por valor
         Minima(TEM, I);
     }
     printf("\nPromedio del dia: %5.2f", (ACT / 24));
     printf("\nMáxima del dia: %5.2f   /tHora: %d", MAX, HMAX);
     printf("\nMinima del dia: %5.2f    /tHora: %d", MIN, HMIN);
 }

 void Acutem(float T)
 //esto acumula las temperaturas en la variable global ACT y luego calcula el promedio
 {
     ACT += T;
 }

 void Maxima(float T, int H)
 //Esta función almacena la temperatura maxima y la hora en que se
